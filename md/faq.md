...
# 1.0.7βで確認している不備
- 『数字アタック』で、最終問題でいれかえをすると直前と同じ問題が出現してしまう
- 『数字アタック』で、16問コースのときにエラーが発生してしまう（13問～以降）

## スマホアプリ版の不備（1.0.6.6β）
- カスタマイズのステージを『ランダム』にした状態で遊ぶとき、中断データを再開するなどで『デフォルト』に変更されてしまうことがある
...
引き継ぎを行うと、ヘルをクリアしたデータ等を保存することができます。
...
### 引き継ぐ先のバージョン の操作
- 上記でコピーしたデータを『www』内に移動することでデータの引継ぎが可能です。
