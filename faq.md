...
<strong style="color:red;font-size:1.1em;">ご迷惑となるため、Plicy様（ブラウザ版を運営されているサイト）の公式連絡フォームから直接『漢字でGO!』のゲーム内容に関するお問い合わせをすることはお控えください。</strong>

また、お問い合わせフォーム以外からの『漢字でGO!』についてのご連絡（『漢字でGO!』開発アカウント、制作者個人アカウントへのDM 等）には一切対応致しかねます。

## 不具合修正の履歴
[<i class="fab fa-twitter"></i> Twitter（X）](https://twitter.com/kanjidego)、漢字でGO! ゲーム内更新履歴に掲載していますので、このページには記載しておりません。
...
## <strong style="color:coral;font-size:1.1em;">ステージを選択し「はじめる！」を選んだあと、まれに『SyntaxError: Unexpected string』と出て進行不可能になる</strong>
現行バージョン(**1.0.0β**)で確認している不具合です。
ただいま原因を調査中です。
...
- 回線状況に問題がある
  - 通信状況のよい場所でお試しください。
- サーバーが混雑している
  - アップデートされたばかりの場合、アクセスが集中しゲームが正常に続行できないおそれがございます。お手数ですが、時間を置いて再度お試しください。
- 推奨環境を満たしていない
  - PCなどのスペックが低いと、予期せぬ不具合が発生します。

## スマホプレイ時にエラーが発生する
上記原因とほぼ同様です。
基本的にPCのプレイを推奨しているゲームのため、スマートフォンではどうしても動作が不安定になってしまう場合がございます。
...
[こちら](#（ブラウザ版）ローディングエラーで開けない)と同じ原因である可能性が高いです。

また、原因を特定しやすくするため、エラー時に表示されるメッセージを詳細にお送りいただけますと有難いです。
...
~~https://twitter.com/KanzideGo/status/1693905210471780577?s=20~~
~~https://twitter.com/KanzideGo/status/1696288997017026817?s=20~~

**ver1.0.1**より、各コースに応じて一定の確率で重複が回避されるようになりました。
...
# 出題内容（ことば編）
## ひらがな以外の●を埋めて答えたら不正解扱いになった
![必ずお読みください画像](./images/kanjidego_kanarazuoyomikudasai.png)  
本ゲームでは、**『ルビの中で開かれているひらがな』も答えに含めて入力します。**

●のみを入力する形式にしてしまうと、解説欄に記載するとき、解答によっていちいち虫食いされた部分を埋めるような仕組みを作らなくてはならないので、問題単位の工数が大幅に増えることを懸念しております。

## 俗語（スラング）が入っているのはいいの？
これらは典拠としているWeblio国語辞典、Wikipedia等に記載されているため、レギュレーション上は出題することができるため収録しております。  
...
<strong style="color:red;font-size:2.1em;">あくまで典拠は『ルールを明確にするための指標』で、『確実性を求めるもの』ではございません。</strong>

そのため多少不明瞭なものが含まれてしまう事をご理解ください。

前提として、問題ごとに具体的な辞典の名前を出すといったことも行いませんので、**ルールの意義を勘違いして本ゲームにそのようなことは求めないでいただきたいです。**

厳粛なルールをご所望でしたらほかのコンテンツを当たっていただきますようお願い申し上げます。
...
典拠サイト側の明らかな脱字・衍字に関しては積極的に差し替え・訂正を行いますが、<strong style="color:red;font-size:1.1em;">『このゲームのルールとして』誤植の可能性があるものをそのまま引用する場合がございます。</strong> 
...
いれかえ、残機機能の実装などもあり、リリース当初よりはかなりクリアしやすくなったとは思いますが、ゲキムズ以降は相応の難易度にするため意図的に難しくしております。**覚えていけばいつかはクリアできるはずです。**
...
## 【検討中】差し替えた問題を教えてほしい
**1.0.1β**以降、**試験的に**更新内容に記載いたします。

[回答日時：2023年10月11日 午後0:20](https://twitter.com/KanzideGo/status/1711763523553124553?s=20)
...
<strong style="color:red;font-size:1.1em;">ステージをひとつ制作するだけでも莫大な時間、工数、労働コストが発生します。
...
<strong style="color:red;font-size:1.1em;">これらを</strong><strong style="color:red;font-size:2.1em;">無償で</strong><strong style="color:red;font-size:1.1em;">作ってほしい／やってほしいと言っていることと同義です。</strong>

これ以上の追加は本当に**要望者からお金を取ろうか**考えましたが、
...
YouTubeでは、デフォルト／アクアのBGM集を投稿しています。  
...
# その他
## 公式で返事がないから何回も送っちゃお～
お問い合わせをしたが公式から回答がないときは、何らかの理由で**意図的にそうしている**場合がございます。

- すでに同じような要望に対して回答している場合
- 現状では答えることが難しい場合
- 非常に多くの工数が掛かるような、現実的な案ではない場合

<strong style="color:red;font-size:1.1em;">何度も送ることによって回答されやすくなるといったことは一切ない</strong>ためご注意ください。


## FAQを確認したが、求めていた項目が見つからなかった
